...
def getDataFromDB(cursor, date, type = 'probird'):
    ...
    if type == 'probird':
        ...
        sql = "SELECT * FROM wind_turbine_data WHERE DATE(Time_Stamp) = '" + date + "' GROUP BY Wind_Turbine_ID, Time_Stamp"
    cursor.execute(sql)
...
def processingRowNumber(date):
    sql = "SELECT Time_Stamp, Wind_Turbine_ID FROM wind_turbine_data WHERE DATE(Time_Stamp) = '" + date +"' GROUP BY Time_Stamp, Wind_Turbine_ID ORDER BY `ID` ASC;"
    cursor.execute(sql)
